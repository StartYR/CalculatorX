...
新增代码以及被实质修改的代码应遵守 [CalculatorX 代码风格指南](./code-style.md)。该指南分别约束 ArkTS/ArkUI、C++/N-API、PowerShell 和 JSON/JSON5，并规定文件头、命名、注释、错误处理与检查边界。

项目采用渐进式统一：不要为了套用新风格而批量格式化或改写本次任务之外的旧代码。
...
新增功能或修复问题的分支，在实现和验证完成后、合并前，应在 [`changes/`](./changes/README.md) 中新增一份变更说明。文档记录最终用户行为、实现边界、实际验证结果和发布说明素材，不用复述开发过程。

发布新 Tag 时，按 Tag 区间筛选新增说明，将“发布说明素材”汇总到 `CHANGELOG.md` 顶部和 GitHub Release，无需重新倒查提交。具体命名、结构和汇总命令见[分支变更说明规范](./changes/README.md)。
