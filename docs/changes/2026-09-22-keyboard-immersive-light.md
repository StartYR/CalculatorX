...
七类键盘均在模块内持有完整的传统与沉浸容器，并在模块内直接读取全局开关和选择路径。公共 Host 与公共渲染选择器虽能通过 ArkTS 构建，但传入的完整布局在真机运行时会丢失 Builder 的 `this` 绑定，因此已经移除。模块继续持有公式区、焦点与动画，便于独立定位问题。
...
公共 Host 和公共渲染选择器版本都曾在 API 26 真机冷启动科学计算时因 `@BuilderParam` 丢失绑定而崩溃。完全移除跨组件布局 Builder 转交后，主包 debug 构建、覆盖安装、科学计算冷启动以及函数图像切换科学计算均已在连接设备上通过，应用进程持续存活且日志没有新的运行时错误。API 23、深浅色、长按、Shift、焦点和动画仍需完整真机回归。
