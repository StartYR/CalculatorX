...
- [x] 新增 `KeyboardKeyRole` 与 `KeyboardKeySpec`；
- [x] 建立语义角色到传统背景的映射；
- [x] 建立语义角色到沉浸材质的映射；
- [x] 为 Shift 激活态提供显式状态转换；
- [x] 为动态按键提供类型安全的构造方式；
- [x] 对映射函数编写纯逻辑测试；
- [x] 删除传统背景与沉浸材质的两套重复分类。
...
- [x] 新增 `KeyboardKeySurface`；
- [x] 把 API 保护和两套 Button 属性链移入公共表面；
- [x] 通过 `@BuilderParam` 保留现有文字和图标内容；
...
- [x] 用统一语义表达矩阵数字、函数、运算、危险、强调和 Shift；
- [x] 使用公共表面替换 `MatrixKeyItem` 内的两套 Button 属性链；
- [x] 保留 `KeyGestureWrapper`、Shift、气泡层级和维度弹窗；
- [x] 删除矩阵模块旧的材质角色分类函数；
- [x] 由统一传统映射保留 `FUNCTION` 与 `OPERATOR` 的背景区别。
...
- [x] 迁移顶部复合按键和底部基础按键；
- [x] 保留上层小字、动态 Shift 标签和 `renderGroup` 策略；
- [x] 保留长按候选、气泡提权和 Shift 自动消费；
- [x] 删除科学模块旧的材质角色分类函数；
- [ ] 真机快速输入并检查公共组件没有增加可感知延迟。
...
- [x] 迁移图形顶部按键、底部按键和定义域按键；
- [x] 保留不同函数类型生成的动态语义；
- [x] 保留单变量跨列、tMin/tMax 焦点、光标和范围同步；
- [x] 保留主键盘与定义域键盘互斥；
- [x] 删除图形模块旧的材质角色分类函数；
- [ ] 真机确认 `TextInput.customKeyboard()` 内仍显示逐键光效。
