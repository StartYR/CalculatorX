...
按当前项目规则，Codex 不执行编译或构建。每一阶段先完成：
...
需要新 HAP 的验证由维护者构建和安装后，在真实设备上继续完成。设备验证必须区分：
...
- AI Agent 可通过场景 JSON 完成主要计算、设置和页面导航测试。
...
任一阶段未达到验证目标时，先修正该阶段，不提前扩展后续能力。复杂手势、MathLive 光标、图像视觉、系统界面和视觉回归始终保持在范围之外。
